...
int CCheckComboBox::GetCheckCount(void)
{
	int nSelected = 0;

	for (int i=0; i<m_CheckArray.GetSize(); i++)
	{
		if (GetCheck(i))
		{
			nSelected++;
...
void CCheckComboBox::OnMouseMove(UINT nFlags, CPoint point) 
{
	static CPoint pointTmp;
	if (point==pointTmp)
	{
...
void CCheckComboBox::OnTimer(UINT nIDEvent) 
{
	CPoint pnt;
	GetCursorPos(&pnt);
	CRect rec;
